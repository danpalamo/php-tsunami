...
function runExternal($cmd) {
  global $runningChildren;
  global $grapherConcurrency;
  ...
  if ($pid == -1) {
    ...
    // parent
    $runningChildren++;
    if ($runningChildren>=$grapherConcurrency) {
      pcntl_wait($status);
      $runningChildren--;
    }
  } else {
...
if($argc > 2)
{
	if($argv[2] == "all" or $argv[2] == "none")
		...
		$searchSubhost = $argv[2];	//use $argv[2] as subhost ID
}
	...
	if(!isset($searchSubhost) or $searchSubhost == "all" or $searchSubhost == "none")
	{
		...
		foreach($host->m_graphs as $graph)
		{
			foreach($aGraphTimeframes as $timeframe)
			{
				...
				$cmdLine .= " --color BACK#00000000 --color CANVAS#ffffff --color ARROW#00000000 --color SHADEA#00000000 --color SHADEB#00000000 --disable-rrdtool-tag";
...
//				echo "\"$cmdLine\".\n" ;
				...
				if(isset($graph->m_bPreview))
				{
					...
					$cmdLine .= " --title=\"\" --vertical-label=\"\" --color BACK#00000000 --color CANVAS#ffffff --color ARROW#00000000 --color SHADEA#00000000 --color SHADEB#00000000 --no-legend --disable-rrdtool-tag";
					echo date("Ymd H:i:s")." Creating graph \"$filenamePNG\"\n";
//					echo "\"$cmdLine\".\n" ;
					runExternal($cmdLine);
				}
	...
	foreach($aCollectedHosts[$host->m_id]->m_subhosts as $subhost)
	{
		...
		foreach($host->m_graphsWalk as $graph)
		{
			foreach($aGraphTimeframes as $timeframe)
			{
				...
				eval("\$cmdLine = \"".addslashes($graph->m_cmdLine)."\";");
				$cmdLine .= " --color BACK#00000000 --color CANVAS#ffffff --color ARROW#00000000 --color SHADEA#00000000 --color SHADEB#00000000 --disable-rrdtool-tag";
				// These graphs are generated in realtime
				// echo date("Ymd H:i:s")." Creating graph \"$filenamePNG\"\n";
				// runExternal($cmdLine);
